...
describe("POST /api/admin/tenant/[tenantId]/feature-flags", () => {
  const buildRequest = (options: { body?: BodyInit | null; json?: unknown } = {}) =>
    makeRequest({ method: "POST", url: "http://localhost/api/admin/tenant/tenant-a/feature-flags", ...options });
  ...
  it("enforces tenant admin access when the caller cannot manage all feature flags", async () => {
    ...
    const response = await POST(buildRequest({ json: { name: "scoring", enabled: true } }), params);
  ...
  it("validates feature flag payloads", async () => {
    ...
    const response = await POST(buildRequest({ json: { name: "", enabled: "yes" } }), params);

    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "name and enabled are required" });
  });
