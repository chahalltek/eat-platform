...
const mockResolveTenantAdminAccess = vi.hoisted(() => vi.fn());
...
vi.mock("@/lib/tenant/access", () => ({
  resolveTenantAdminAccess: mockResolveTenantAdminAccess,
}));
...
describe("GET /api/admin/tenant/[tenantId]/diagnostics", () => {
  beforeEach(() => {
    ...
    mockResolveTenantAdminAccess.mockResolvedValue({ hasAccess: true, isGlobalAdmin: true, membership: null });
  });
  ...
  it("enforces tenant or global admin access", async () => {
    mockResolveTenantAdminAccess.mockResolvedValue({ hasAccess: false, isGlobalAdmin: false, membership: null });
