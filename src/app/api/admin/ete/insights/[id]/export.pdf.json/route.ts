...
import { buildPdfReadyInsight } from '@/lib/publishing/renderInsightContent';
...
export async function GET(
  ...
  try {
    ...
    const pdfContent = buildPdfReadyInsight(snapshot);

    return NextResponse.json(pdfContent, {
      status: 200,
      headers: {
        'Content-Disposition': `attachment; filename="insight-${snapshot.id}-pdf.json"`,
      },
    });
  } catch (error) {
