...
import { GET } from "@/app/api/admin/ete/tests/route";
...
describe("GET /api/admin/ete/tests", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns the catalog for admins", async () => {
    ...
    await withListeningServer(server, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/api/admin/ete/tests`);
      ...
      expect(body).toEqual(getEteTestCatalog());
    });
  });
